...
#pragma pack(pop)
    ...
    public: /* Modifiers */
        ...
        */
        Bitmap& flip_v_self() {
            Bitmap temp(m_width, m_height);
            for (std::int32_t x = 0; x < m_width; ++x) {
                for (std::int32_t y = 0; y < m_height; ++y) {
                    // Calculate the reverse y-index
                    temp.m_pixels[IX(x, y)] = m_pixels[IX(x, m_height - 1 - y)];
                }
            }
            *this = std::move(temp); // Update current object
            return *this;
        }
        ...
        Bitmap& flip_h_self() {
            ...
            for (std::int32_t y = 0; y < m_height; ++y) {
                for (std::int32_t x = 0; x < m_width; ++x) {
                    ...
                    temp.m_pixels[IX(x, y)] = m_pixels[IX(m_width - 1 - x, y)];
                }
            }
            *this = std::move(temp); // Update current object
            return *this;
        ...
        */
        Bitmap& rotate_90_right_self() {
            ...
            for (std::int32_t y = 0; y < m_height; ++y) {
                std::int32_t y_offset = y * m_width; // Precompute row start index
                for (std::int32_t x = 0; x < m_width; ++x) {
                    temp.m_pixels[x * m_height + (m_height - 1 - y)] = m_pixels[y_offset + x];
                }
            }
    ...
    public: /* Utils */
        ...
        HBITMAP ConvertToHBITMAP() {
            ...
            bmpInfo.bmiHeader.biHeight = -m_height; // Negative to indicate a top-down DIB
            bmpInfo.bmiHeader.biPlanes = 1;
            bmpInfo.bmiHeader.biBitCount = sizeof(Pixel) * 8;
            ...
            // Assuming m_pixels is stored as RGBA (32-bit) in row-major order
            const size_t dataSize = m_width * m_height * 3; // 4 bytes per pixel (RGBA)
            memcpy(bits, m_pixels.data(), dataSize);
